...
$result = $conn->query("SELECT nombre FROM hospedajes");
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $hospedajes[] = $row['nombre'];
    }
}
...
<head>
    ...
    <title>Reservar Hospedaje</title>
</head>
<body>
    <div class="sesion">
        <form action="guardar_reserva_h.php" method="POST">
            <h1>Reserva de Hospedaje</h1><br>
            ...
            <label for="hospedaje">Seleccione el hospedaje:</label>
            <select name="hospedaje" required>
                <option value="">-- Seleccione --</option>
                <?php foreach ($hospedajes as $nombre_hosp): ?>
                    <option value="<?php echo htmlspecialchars($nombre_hosp); ?>">
                        <?php echo htmlspecialchars($nombre_hosp); ?>
                    </option>
                <?php endforeach; ?>
            </select><br><br>
            
            <input type="submit" value="Reservar">
            <button class="btn-volver" type="button" onclick="location.href='hospedaje.php'">Volver</button>
        </form>
    </div>
    <footer>
        <div class="datos">
